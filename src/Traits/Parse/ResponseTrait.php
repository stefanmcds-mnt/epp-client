    ...
    public function _ParseResponseBody(?string $xml, ?string $element = null)
    {
        ...
        if ($res = XML2Array::createArray($xml)) {
            ...
            if (isset($res['greeting'])) {
                $this->registry = CommandTrait::_Greeting(parse: $res['greeting']);
            }

            // Epp msgQ
            if (isset($res['msgQ'])) {
                // store msgQ
                $res['msgQ'] = CommandTrait::_msgQ(parse: $res['msgQ']);
            }

            // The Epp Result Element
            if (isset($res['result'])) {
                // store result
                $res['result'] = resultTrait::_Result(parse: $res['result']);
            }
            ...
            if (isset($res['resData'])) {
                ...
                if (isset($res['resData'][$element . ':trnData'])) {
                    // store new item key
                    $res['trnData'] = resDataTrait::_trnData(parse: $res['resData'][$element . ':trnData'], element: $element);
                    // remove old
                ...
                if (isset($res['resData'][$element . ':infData'])) {
                    ...
                    $res['resData'] = resDataTrait::_infData(parse: $res['resData'][$element . ':infData'], element: $element);
                    // Status
                    if (isset($res['resData'][$element . ':status'])) {
                        // store status
                        $res['resData'][$element . ':status'] = resDataTrait::_infData_status(parse: $res['resData'][$element . ':status'], element: $element);
                    }
                    // Contact
                    if (isset($res['resData'][$element . ':contact'])) {
                        // store to element contact
                        $res['resData'][$element . ':contact'] = resDatatrait::_infData_contact(parse: $res['resData'][$element . ':contact'], element: $element);
                    }
                    // NS on command Domain
                    if (isset($res['resData'][$element . ':ns'])) {
                        // store element ns
                        $res['resData'][$element . ':ns'] = resDataTrait::_infData_ns(parse: $res['resData'][$element . ':ns'], element: $element);
                    }
                    // Authinfo
                    if (isset($res['resData'][$element . ':authInfo'])) {
                        // store element authInfo
                        $res['resData'][$element . ':authInfo'] = resDataTrait::_infData_authInfo(parse: $res['resData'][$element . ':authInfo'], element: $element);
                    }
                    // PostalInfo on command Contact
                    if (isset($res['resData'][$element . ':postalInfo'])) {
                        $res['resData'][$element . ':postalInfo'] = resDataTrait::_infData_postalInfo(parse: $res['resData'][$element . ':postalInfo'], element: $element);
                    }
                    // voice on Contact Command
                    if (isset($res['resData'][$element . ':voice'])) {
                        // store voice key
                        $res['resData']['voice'] = resDataTrait::_infData_Tel(parse: $res['resData']['voice'], element: $element);
                    ...
                    if (isset($res['resData'][$element . ':fax'])) {
                        // store fax key
                        $res['resData']['fax'] = resDataTrait::_infData_Tel(parse: $res['resData']['fax'], element: $element);
                    ...
                    if (isset($res['resData'][$element . ':chkData'])) {
                        ...
                        $res['resData'][$element] = resDataTrait::_chkData(parse: $res['resData'][$element . ':chkData'], element: $element);
                        // delete original
                    ...
                    if (isset($res['resData'][$element . ':creData'])) {
                        ...
                        $res['resData'] = array_merge($res['resData'], resDataTrait::_creData(parse: $res['resData'][$element . ':creData']));
                        // delete original
                ...
                if ($element === 'session') {
                    foreach ($ext as $e) {
                        ...
                        if (isset($res['extension'][$e . ':creditMsgData'])) {
                            $res['creditMsgData'] =  extensionTrait::_creditMsgData(parse: $res['extension'][$e . ':creditMsgData'], ext: $e);
                            unset($res['extension'][$e . ':creditMsgData']);
                        }
                        // Password Reminder
                        if (isset($res['extension'][$e . ':passwordReminder'])) {
                            $res['passwordReminder'] = extensionTrait::_passwordReminder(parse: $res['extension'][$e . ':passwordReminder']);
                            unset($res['extension'][$e . ':passwordReminder']);
                        }
                        //delayedDebitAndRefundMsgData
                        if (isset($res['extension'][$e . ':delayedDebitAndRefundMsgData'])) {
                            $res['delayedDebitAndRefundMsgData'] = extensionTrait::_delayedDebitAndRefundMsgData(parse: $res['extension'][$e . ':delayedDebitAndRefundMsgData']);
                            unset($res['extension'][$e . ':delayedDebitAndRefundMsgData']);
                        }
                        // simpleMsgData
                        if (isset($res['extension'][$e . ':simpleMsgData'])) {
                            $res['simpleMsgData'] = extensionTrait::_simpleMsgData(parse: $res['extension'][$e . ':simpleMsgData']);
                            unset($res['extension'][$e . ':simpleMsgData']);
                        }
                        //wrongNamespaceReminder
                        if (isset($res['extension'][$e . ':wrongNamespaceReminder'])) {
                            $res['wrongNamespaceReminder'] = extensionTrait::_wrongNamespaceReminder(parse: $res['extension'][$e . ':wrongNamespaceReminder'], ext: $e);
                            unset($res['extension'][$e . ':wrongNamespaceReminder']);
                        }
                        // dnsErrorMsgData
                        if (isset($res['extension'][$e . ':dnsErrorMsgData'])) {
                            $res['dnsErrorMsgData'] = extensionTrait::_dnsErrorMsgData(parse: $res['extension'][$e . ':dnsErrorMsgData'], ext: $e);
                        }
                        // chgStatusMsgData 
                        if (isset($res['extension'][$e . ':chgStatusMsgData'])) {
                            $res[$element . ':chgStatusMsgData'] = extensionTrait::_chgStatusMsgData(parse: $res['extension'][$e . ':chgStatusMsgData'], ext: $e);
                            unset($res['extension'][$e . ':chgStatusMsgData']);
                        }
                        // dlgMsgData
                        if (isset($res['extension'][$e . ':dlgMsgData'])) {
                            $res['dlgMsgData'] = extensionTrait::_dlgMsgData(parse: $res['extension'][$e . ':dlgMsgData'], ext: $e);
                    ...
                    if (isset($res['extension'][$ext . ':trade'])) {
                        $res['trnData'] = extensionTrait::_trade(parse: $res['extension'][$ext . ':trade'], ext: $ext);
                    ...
                    if (isset($res['extension'][$ext . ':infContactsData'])) {
                        $res[$element . ':infContacts'] = extensionTrait::_infContactsData(parse: $res['extension'][$ext . ':infContactsData'], ext: $ext);
                    ...
                    if (isset($res['extension'][$ext . ':infData'])) {
                        ...
                        if (isset($res['extension'][$ext . ':infData'][$ext . ':ownStatus'])) {
                            $res[$element . ':ownStatus'] = extensionTrait::_infData(parse: $res['extension'][$ext . ':infData'], ext: $ext);
                        ...
                        if (isset($res['extension'][$ext . ':infData'][$ext . ':consentForPublishing'])) {
                            $res[$element . ':consentForPublishing'] = extensionTrait::_infData(parse: $res['extension'][$ext . ':infData'], ext: $ext);
                            unset($res['extension'][$ext . ':infData'][$ext . ':consentForPublishing']);
                        }
                        // Contact Registrant
                        if (isset($res['extension'][$ext . ':infData'][$ext . ':registrant'])) {
                            $res = array_merge($res, extensionTrait::_infData(parse: $res['extension'][$ext . ':infData'], ext: $ext));
                    ...
                    if (isset($res['extension'][$ext . ':infNsToValidateData'])) {
                        $res[$element . ':nsToValidate'] = extensionTrait::_infNsToValidateData(parse: $res['extension'][$ext . ':infNsToValidateData'], ext: $ext);
                        unset($res['extension'][$ext . ':infNsToValidateData']);
                    }
                    // SecDNS
                    if (isset($res['extension']['secDNS:infData'])) {
                        $res['secDNS'] = extensionTrait::_secDNS_infData(parse: $res['extension']['secDNS:infData']);
                    ...
                    if (isset($res['extension']['extsecDNS:infDsOrKeyToValidateData'])) {
                        if (isset($res['extension']['extsecDNS:infDsOrKeyToValidateData']['dsOrKeysToValidate'])) {
                            $res['secDNS']['dsOrKeysToValidate'] = extensionTrait::_extsecDNS_infDsOrKeyToValidateData(parse: $res['extension']['extsecDNS:infDsOrKeyToValidateData']['dsOrKeysToValidate']);
                        } else {
                            $res['secDNS']['remAll'] = extensionTrait::_extsecDNS_infDsOrKeyToValidateData(parse: $res['extension']['extsecDNS:infDsOrKeyToValidateData']);
                        }
            ...
            if (isset($res['trID'])) {
                $res = array_merge($res, CommandTrait::_trID(parse: $res['trID']));
                unset($res['trID']);
            }
            // Set $xmlResult
